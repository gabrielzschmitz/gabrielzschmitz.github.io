...
Even though the values $ g $, $ p $, $ A $, and $ B $ are publicly visible, the
attacker still doesn’t know the private values $ a $ or $ b $.
...
There’s no efficient shortcut to reverse modular exponentiation without knowing
the secret exponent. In fact:
